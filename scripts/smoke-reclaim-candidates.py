...
def main():
    ...
    with App() as app, fixture["Fixture"](item) as mock:
        ...
        policy = dict(enabled=True, ruleIds=[saved["id"]], maxDeletes=1, maxReclaimBytes=2**30, maxRecentUploadBytes=1000, recentUploadWindowSeconds=60, maxOvershootBytes=2**30)
        ...
        if not result["candidates"]:
            print("OBSERVATION DIAGNOSTICS", app.rows("SELECT elapsed_ns,duration_ns,observed_at FROM reclaim_condition_observations"), app.rows("SELECT MIN(observed_ns),MAX(observed_ns),COUNT(*) FROM reclaim_upload_samples"), flush=True)
        assert [c["hash"] for c in result["candidates"]] == [item["hash"]], result
        assert result["candidates"][0]["recentUploadBytes"] == 0
        assert result["candidates"][0]["uploadWindowSeconds"] == 60
