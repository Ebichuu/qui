...
| `racingASNDatabasePath` | `QUI__RACING_ASN_DATABASE_PATH` | string | empty | Optional offline GeoLite2-ASN MMDB (up to 64 MiB). Relative paths resolve against the config directory. Restart after changing the path or database. An unreadable or invalid database leaves Peer sampling active without new ASN results. |
