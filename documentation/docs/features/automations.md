...
**Optional condition match duration:** Set `conditionMatchDurationSeconds` to require the delete condition to remain matched across consecutive workflow runs before deletion. The minimum non-zero value is 60 seconds; omit it or use `0` for immediate deletion. Non-matches, failed observations, stale sync data, counter resets, task re-additions, rule changes and long gaps between observations reset the timer. Cooldown suppresses deletion while observation continues. Choose a workflow interval no longer than the duration so qui can observe the condition, and keep it short enough to detect interruptions. Completed observation checkpoints survive a restart: a short restart preserves previously measured time but does not count time offline. A gap longer than twice the workflow interval resets the timer. An interrupted scan does not restore its previous qualification. The existing five-minute FREE_SPACE cooldown is recorded before a delete request, so failed or ambiguous requests also retain that cooldown across restarts.

**Optional daily trigger:** The delete action can use a separate `dailyTrigger` expression in addition to its main `condition`. Daily deletion requires both, but sustained observation counts only the main condition. For example, a low-upload condition can accumulate observed time while a separate FREE_SPACE trigger is false. When space becomes low, the existing measured time remains available. Cooldown still blocks daily deletion while observation continues. The workflow editor provides a separate optional trigger field; leaving it empty preserves existing behavior. Existing combined expressions are never split automatically. Changing the rule resets its observation qualification. This separation does not authorize official reclaim or start another deletion executor.
...
Official reception events with explicit reclaim permission can now produce a read-only shortage assessment when no direct reception target has enough capacity. The latest 100 assessments are available at `/api/racing/reclaim-assessments`. Records retain their observation time and configuration revision across restart. They are historical explanations, not active reservations.

Assessments preserve an existing negative capacity balance and never treat unknown release or upload-window evidence as zero cost. The combination evaluator checks the entire deficit and all budgets before returning a selection; it returns no partial list when capacity is insufficient. This stage does not connect measured physical release, persistent spending, site protection or deletion execution. Actual free space becoming available still resumes the normal add path.

Candidate observations now use real qB sync timestamps rather than repeated API-read time or optimistic UI state. Recent upload counters persist for the configured window plus a bounded sample gap. A covered window reports a conservative byte upper bound including its enclosing edge interval; missing coverage omits the value. Observed counter rollback, task replacement, disappearance or a long gap starts a new window. A reset that occurs and recovers entirely between samples is not detectable from those snapshots.

On Linux amd64/arm64 with local filesystem access, read-only assessment can inspect native ext4, XFS or Btrfs file extents. Shared, pending, encoded or unsupported allocations remain unknown, as do hardlinked files, incomplete inventories, filesystem boundaries and stale observations. Other platforms retain unknown capacity. A viable capacity combination still waits for site protection; extent evidence is an estimate before deletion, never confirmation that space has been released.


A complete, protected combination can also prepare a persistent reclaim plan. `GET /api/racing/reclaim-plans` lists the latest 100 plans with their frozen budget ceilings and charged amounts. Raising settings or restarting cannot reset the original budget; lower current limits still apply. A charge represents a conservative amount recorded at the deletion-claim boundary, not measured released space. Plans keep their target instance and physical pool, and an unresolved step cannot be overwritten. The runtime currently stops at preparation: no official reclaim network executor is enabled. Task disappearance alone does not confirm physical release.
