...
## Tracker account rules

Open **Automations → Reannounce → Configure → Tracker account rules**. Rules save separately from monitoring settings. Select a saved Tracker identifier and an enabled site with its hostname. For a new binding or a changed site, paste the full account Tracker address to verify it locally against that identifier. The address is never submitted. Verify that it belongs to the selected site account; hostname matching alone is insufficient. HTTPS or localhost is required for this local check.

For a known wait, enter the exact complete response, including whitespace. Its digest is computed locally; only the digest is submitted. Leaving it blank while editing retains the saved match. Turning off the known-wait option removes that match without canceling an existing deadline.

`GET` and `PUT /api/instances/{instanceID}/reannounce/tracker-policies` remain available for listing and updating rules. GET includes invalid site bindings so they can be repaired; execution still rejects them. Never publish full announce URLs or messages containing credentials. A host alone cannot identify an account.

Each policy requires `trackerKey`, `trackerHost`, an enabled `siteId` whose configured hosts include that host, and `intervalSeconds` (1–2,592,000). `waitMessageDigest` plus `waitSeconds` configures one explicitly recognized message and its known wait; use an empty digest and zero seconds when no message is configured. Unrecognized errors do not acquire an inferred site-specific wait. Policies can be updated but do not yet have a removal endpoint. Disabling their site or removing its host blocks execution until the binding is repaired. The editor continues to display those rules.
