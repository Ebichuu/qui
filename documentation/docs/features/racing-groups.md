...
Periodic observation of an already confirmed torrent does not repeat its identity confirmation log. These records omit private Tracker URLs and transport errors. Candidate decisions are logged at Info only when their persisted selection changes, including source removal. Candidate records include the source title; add and confirmation records include the original verified metainfo name. The existing log viewer searches timestamps, names, hashes and IDs in its loaded history; older records require configured file logging and retained log files. This is not an unlimited historical query service.

### Group selection and decision evidence

Eligibility checks still enforce fresh observations, reception permissions, concurrency limits, storage mappings and available capacity. Among eligible targets, Racing compares the larger of current upload and download speed. Targets within the larger of 1 MiB/s or 10% of the lowest load form a comparable-load group. Within that group, fewer active and pending tasks win, followed by the least recently reserved instance. Completed reception history also participates. A clearly busier instance or one without enough capacity cannot win through history compensation.

Each new reservation stores the public candidate, winning rule selection, algorithm version, ordered eligible targets and selected position in `plan.decision`. Target evidence includes observation time, both transfer speeds, active and pending counts, available bytes after commitments and the configured free-space floor, storage pools and last reservation time. The verified original name is stored in `plan.name`. Later candidate updates, Peer/ASN observations and configuration changes do not rewrite a submitted plan. Existing intents created before this feature have no decision snapshot. This snapshot explains eligible-target ordering; it does not contain a complete copy of every rejected instance or the whole torrent library.

### Acceptance image publication

The Racing CI workflow accepts an optional `publish_image` input when manually run on `racing/develop`. It publishes a Linux amd64 image only after the full validation job succeeds. Images use `ghcr.io/ebichuu/qui:racing-<full-commit>` and the workflow summary records the immutable digest. This does not update the production `latest` tag. Pin the digest when deploying an acceptance instance, use a separate configuration directory, and bind its port to loopback until access is configured.

Never start a production database copy with live automation or downloader connections enabled. Keep the original consistent backup intact and disable instance polling and automation in the disposable copy before starting the new version. Validate migration and restoration there before transferring any execution authority.
