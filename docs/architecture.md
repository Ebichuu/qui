...
### Persistent reclaim plans and charges

A feasible protected assessment can prepare a plan with a fixed target and initial policy ceilings. The event deadline cannot be extended by replanning. Plan rows deliberately outlive discovery/configuration cleanup, so removing a source cannot erase an already charged budget. Reassessment uses the persisted frozen policy and charges; current lower ceilings still apply. Prepared snapshots expire and must be reassessed before a claim.

The store claim boundary shares the automatic-delete transaction: one first item, its conservative charge and the common delete intent commit together. Case-insensitive hash ownership prevents differently cased callers from claiming twice. An unresolved step blocks rewriting the plan, and confirming torrent absence never refunds the charge. Plans do not reserve physical capacity or certify release. The execution runner consumes a prepared plan only with the target instance’s explicit reclaim permission. It rechecks the deficit and candidate eligibility, captures a native baseline, and enters the common guarded single-attempt delete path. The transaction also compares the pool commitment fingerprint; concurrent reception changes require reassessment.


The claim also stores a pre-delete file/space baseline and a pending pool lease. A second claim on that pool waits until task absence is confirmed and a fresh net-capacity observation is recorded. The next baseline must be newer than the previous observation. Net growth is not credited to an independent capacity balance: reception still reads current pool availability and outstanding promises. A release receipt retains all charges and clears the old selection, requiring complete reassessment before another step. Unknown request results cannot be confirmed by task absence. Upgrades retain old unresolved pool occupancy with an unknown baseline; they never reconstruct pre-delete evidence from current free space. A separate bounded reconciliation worker revisits accepted/confirmed operations across restarts, even after reclaim permission is disabled. It checks native file absence and net capacity before recording the receipt. Unknown sends retain ownership and are not automatically completed. The same event cannot reserve an add while its reclaim step awaits release.

### Racing Peer analysis

Peer history uses an independent opt-in instance setting and one sequential worker. It never acquires execution slots or writes reception decisions. A persisted confirmed intent defines a fixed 20-minute observation window; missing and partial samples remain in the five-second coverage denominator, even when no history row was written. Exact verified hashes resolve the qB index hash, with added_on checked before and after each peer request. IP/port endpoint history and attempt states are bounded to 1000 endpoints and 240 attempts per candidate and pruned after 30 days. A successful complete snapshot alone establishes endpoint absence. Counter resets and generation changes do not become continuous transfer evidence. An optional GeoLite2-ASN MMDB is loaded and verified inside the analysis worker from a bounded immutable snapshot. ASN lookups use only local IP data, preserve database build time and SHA-256 identity, and do not alter Peer sample coverage or execution decisions. Configuration and database replacement require a restart; failed loads leave sampling active. Completed histories keep their original evidence. Site rankings remain unavailable; neither ASN ownership nor local upload implies site accounting.

### Reception decision evidence

New reservations embed immutable public candidate/selection JSON and ordered eligible-target evidence in plan_json. This uses the existing atomic reservation and revision fences; no separate history store or migration is introduced. Current candidate selection remains mutable. The algorithm identifier records comparable-load selection using max(upload, download), then active/pending tasks and oldest reservation; hard capacity and permission gates run first. Logs compare against the persisted previous selection so restarts do not reset decision-log suppression.
