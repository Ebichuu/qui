...
Reclaim observations use a real successful qB snapshot, excluding optimistic UI state. Upload windows retain bounded persistent samples by task generation; gaps and observed counter regression invalidate coverage. Linux local-file evidence requires exclusive native extents and complete, fresh inventories across configured pool members. Unsupported filesystems and uncertain ownership remain unknown. These values are estimates for assessment, not confirmation of physical release. Even a feasible combination waits for site protection; no reclaim deletion is dispatched.
