...
AutomaticDelete invokes the shared service guard before its persistent ownership claim and rechecks local generation afterward. Unmanaged daily tasks preserve existing behavior. Reclaim assessment requires complete coverage and otherwise keeps candidates protected. The explicit reported_working condition accepts only qB state; blocked and accounted never infer site-accounting evidence. A feasible protected assessment advances only to awaiting_execution_plan, not deletion. Policy configuration currently has an API but no editor or removal endpoint.
