...
func (s *Service) SampleInstance(ctx context.Context, instanceID int) (*models.DailyTransferStats, error) {
	...
	client, err := s.clientPool.GetClientOffline(ctx, instanceID)
	if err != nil {
		return nil, fmt.Errorf("connect to qBittorrent: %w", err)
	}
	syncManager := client.GetSyncManager()
	if syncManager == nil || syncManager.LastSuccessfulSyncTime().IsZero() {
		return nil, errors.New("qBittorrent has no successful shared sync sample")
	}
	observedAt := syncManager.LastSuccessfulSyncTime()
	...
	if !observedAt.Equal(syncManager.LastSuccessfulSyncTime()) {
		return nil, errors.New("qBittorrent sample changed while reading counters")
	}
	return s.store.Record(ctx, instanceID, observedAt.In(s.location), state.AlltimeDl, state.AlltimeUl)
}

// Current consumes the shared sync cache without making a qBittorrent request.
// Only today's persisted value is returned; yesterday is never presented as today.
func (s *Service) Current(ctx context.Context, instanceID int) (*models.DailyTransferStats, error) {
	...
	if sampleErr == nil && stats.Date == time.Now().In(s.location).Format("2006-01-02") {
		return stats, nil
	}
