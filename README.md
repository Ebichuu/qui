...
更新日期：2026-09-10。C00—C02 底座升级、数据库衔接和既有定制恢复已实现，通过 SQLite/PostgreSQL 升级与隔离页面验证。C03—C07 只读配置、共享状态、群组/存储池、独立来源发现、完整规则选择与原 RSS 页面改造已实现，完整竞速功能尚未完成，当前版本不用于替换现有部署。
...
[公开开发计划](DEVELOPMENT.md) 列出 C00—C14 的顺序、交付范围和执行边界；正在推进 C08 选机、空间预留与可靠添加。
