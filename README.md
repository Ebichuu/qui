...
更新日期：2026-09-11。C00—C02 底座升级、数据库衔接和既有定制恢复已实现，通过 SQLite/PostgreSQL 升级与隔离页面验证。C03—C07 只读配置、共享状态、群组/存储池、独立来源发现、完整规则选择与原 RSS 页面改造已实现，C08 默认关闭的接种、共享空间预留及恢复已通过隔离验证。完整竞速功能尚未完成，当前版本不用于替换现有部署。
...
- [共享状态与群组](development-c04.md)：后台同步、状态新鲜度、物理存储池和路径映射。
- [可靠接种记录](development-c08.md)：独立选机、共享预留、发送意图和超时/重启核实。
...
[公开开发计划](DEVELOPMENT.md) 列出 C00—C14 的顺序、交付范围和执行边界；C09 已补充双来源、通知/下载器隔离和崩溃恢复测试；本机性能记录见 [C09 验证记录](development-c09.md)，实际站点及 Vertex 对照待现场验证。
